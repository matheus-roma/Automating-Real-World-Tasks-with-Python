...
if __name__=='__main__':
    ...
    if small:
        print('expat tree:')
        pp(tree)
    from rlextra.radxml.xmlmap import xml2TT
    print(f'Try 2: with {xml2TT.xmlParser} treebuilder')

    #try comparison
    start = time.perf_counter()
    tree2 = xml2TT(xmlText)
    finish = time.perf_counter()
    print('parsed %d bytes in %0.4f seconds' % (len(xmlText), finish-start))
    if small:
        print(f'{xml2TT.xmlParser} tree:')
        pp(tree2)
    ...
    if tree == tree2:
        ...
        if not small:
            ...
            pp(tree2,stream=open(f'/tmp/{xml2TT.xmlParser}_tree.txt','w'))
