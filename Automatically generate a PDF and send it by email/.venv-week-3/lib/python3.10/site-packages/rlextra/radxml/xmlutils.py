...
import os, re
from reportlab.lib.utils import strTypes, asUnicode, asBytes, isUnicode
...
class TupleTreeWalker:
    """Iterates over a tuple tree generating events.
...
class NonEscapingTagWrapper(TagWrapper):
    @staticmethod
    def xmlEscape(a):
        if isinstance(a,bytes): return quotedBytes(a)
...
class SilentTagWrapper(TagWrapper):
    ...
    >>> p = xml2TT("<document id='123'><body>Hello World</body></document>")
...
def validateXhtml(source, rootElement="html", wrapIfNeeded=True):
    ...
    Traceback (most recent call last):
    """+xml2TT.__DTERROR__+""" Undeclared attribute this for element head
    ...
    Traceback (most recent call last):
    """+xml2TT.__DTERROR__+""" Content model for p does not allow element p here
    ...
    return xml2TT(source,eoCB=xhtml.openEntity,
            attribute_defaults=True, resolve_entities=True, load_dtd=True, dtd_validation=True)

def validateXhtmlDocument(docText):
    """Parse, with validation, an XHTML 1.0 Strict document and return tuple tree.
    ...
    Traceback (most recent call last):
    """+xml2TT.__DTERROR__+""" Start tag for undeclared element boo
    ...
    ... #doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
    """+xml2TT.__DTERROR__+""" Undeclared attribute border for element img
    ...
    """
    from rlextra.radxml import xhtml
    return p.parse(docText,eoCB=xhtml.openEntity)

def validateXhtmlTransitionalDocument(docText):
    """Parse, with validation, an XHTML 1.0 Transitional document and return tuple tree.
    ...
    """
    from rlextra.radxml import xhtml
    return p.parse(doctext,eoCB=xhtml.openEntityTransitional)
...
def test(gentests=False):
    ...
    if sys.version_info[0]>2:
        ...
        doctest.testmod()
    return R.__self__ if gentests else None
