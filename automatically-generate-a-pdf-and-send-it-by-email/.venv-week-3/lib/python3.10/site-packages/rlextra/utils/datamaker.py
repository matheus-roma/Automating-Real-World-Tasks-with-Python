...
class DataMaker:
    ...
    def _getGroupedData(self,grpCol):
        G = {}
        S = []
    ...
    def fetchNextDataSet(self):
        ...
        if not self._data:
            return self._groupedData.pop(0) if self._groupedData else None
        else:
            grpCol = self.name2Column(self.groupingColumn)
            if grpCol is not None:
                self._getGroupedData(grpCol)
                return self.fetchNextDataSet()
            #each row is a block
...
class CSVDataMaker(DataMaker):
    ...
    def connect(self,verbose=0):
        pythonCSV = self.csv_reader_kwds is not None
        if pythonCSV:
            with open(self.filename,'rb') as f:
                import csv, io
                from .strutils import autoDecode
                sf = io.StringIO(autoDecode(f.read()))
            csv_reader_kwds = self.csv_reader_kwds
            if 'delimiter' not in csv_reader_kwds and self.sep!=',':
                csv_reader_kwds = csv_reader_kwds.copy()
                csv_reader_kwds['delimiter'] = self.sep
            r = csv.reader(sf,**csv_reader_kwds)
        ...
        if n>0:
            ...
            if j is not None:
                ...
                names = headers[j]
                if not pythonCSV:
                    _processLine(headers[j],self.sep)
        else:
...
if __name__=='__main__':
    ...
    f = StringIO('''chartId\tFundId\tReturn\tVolatility
