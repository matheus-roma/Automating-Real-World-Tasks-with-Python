...
__version__='3.3.0'
import sys, os, fnmatch, re, inspect, time, traceback
from io import StringIO
from reportlab.lib.utils import rl_exec, isNonPrimitiveInstance, annotateException, safer_globals
import tokenize
...
_tg0 = 0
_ofile = os.environ.get('MUTILS_DUMPFILE','')
_ofile = open(_ofile,'w') if _ofile else None
...
def getAttributes(obj,private=0,advanced=0):
	...
	if attrMap:
		for k,a in attrMap.items():
			...
			try:
				cond = (not k.startswith('_') or private) and (hasattr(obj,k) and (not advancedUsage or advanced))
			except:
				cond = False
			if cond:
