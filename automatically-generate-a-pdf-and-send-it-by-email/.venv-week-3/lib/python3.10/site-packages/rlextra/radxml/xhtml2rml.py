...
import os, sys, re
from rlextra.radxml.xmlmap import LazyAttrs, MapController, MapNode, SpecialTransform, NodeTransform
from rlextra.radxml.xml2tt import xml2TT
from rlextra.radxml.xmlutils import TupleTreeWalker
from reportlab.lib.utils import asUnicode, isStr
...
def liTransform(c, liParaStyle='normal', tagAttrs={}):
    "handle list in list"
    s = '<para%s>' % extractTagAttrs('para',tagAttrs,style=liParaStyle)
    e = '</para>'
    x = c.find('<ul')
    if x<0: x = c.find('<ol')
    if x<0: return s + c + e
    ...
    if x<0:
        ...
        c = c[:x]
        if c1: c1 = s + c1 + e
    return ''.join((c0,c,c1)).replace('<br>','<br/>').replace(e+'<br/>'+s,'<br/>')

class TdTransform(NodeTransform):
...
class ImageTransform(SpecialTransform):
    ...
    def __call__(self, sdict, name):
        ...
        if allowMissing:
            if callable(allowMissing) and not allowMissing(src):
                return ''
...
def xhtml2rml(xml, paraStyle='normal', tableStyle='noPaddingStyle', bulletStyle='bullet',
    ...
    M["td"] = '<td>%(__content__)s</td>'
    M["th"] = M["td"]
    M["td"].node_transform = TdTransform()
    ...
    if useModernLists:
        ...
        M["ul"] = '<ul%%(__special_attrs__)s%s>%%(__content__)s</ul>' % extractTagAttrs('ul',tagAttrs,style=ulStyle)
        M["ol"] = '<ol%%(__special_attrs__)s%s>%%(__content__)s</ol>' % extractTagAttrs('ol',tagAttrs,style=olStyle)
        M["li"] = '<li%s>%%(__content__)s</li>' % (extractTagAttrs('li',tagAttrs))
        M["li"].transformContent(curry(liTransform, liParaStyle=liParaStyle))
    else:
        #oldstyle for now
        M["ul"] = '%(__content__)s'
        M["ol"] = '%(__content__)s'
    ...
    if xml2TT.xmlParser=='lxml':
        pkwds = dict(resolve_entities=False)
    else:
        pkwds = dict(ExpandCharacterEntities=0, ExpandGeneralEntities=0)
    xml = xml2TT(xml,**pkwds)
...
def findImages(xml):
    ...
    if xml2TT.xmlParser=='lxml':
        ...
        pkwds = dict(ReturnUTF8=1, ExpandCharacterEntities=0, ExpandGeneralEntities=0)
    tree = xml2TT(validDoc,**pkwds)
