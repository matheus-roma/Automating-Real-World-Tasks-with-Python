...
MatchFail = "MatchFail"
from string import (whitespace as string_whitespace, ascii_letters as string_letters,
                    digits as string_digits, ascii_uppercase as string_uppercase)
...
def xmlgram():
    ...
    # [3]     S ::=  (#x20 | #x9 | #xD | #xA)+
    S = G["S"] = Discard(AnyChars(string_whitespace))
    # NameChar ::=  Letter | Digit | '.' | '-' | '_' | ':' | CombiningChar | Extender
    #   XXX leaving out CombiningChar | Extender for now
    NameChar = G["NameChar"] = AnyChars(string_letters+string_digits+".-_")
    # Nmtoken ::= (NameChar)+
    G["Nmtoken"] = NameChar
    # Name ::= (Letter | '_' | ':') (NameChar)*
    Name = G["Name"] = AnyChars(string_letters+"_:") & (~NameChar)
    ...
    G["VersionNum"] = AnyChars(string_letters+string_digits+".:-")
    # Misc ::= Comment | PI | S
    ...
    G["EncName"] = AnyChars(string_letters) & AnyChars(string_letters+string_digits+"._-")
...
def test2():
    Discard(-AnyChars(string_whitespace))
    num = AnyChars(string_digits)
    N = NamedNode
    G = Grammar("sum")
    t = N("term")
    s = N("sum")
...
def test1():
    ...
    yes(thisorthat, "that")
    no(thisorthat, "other")
    uppers = AnyChars(string_uppercase)
    yes(uppers, "THIS")
