...
embedded flowables, and underlining
"""
import string
from reportlab.pdfbase.pdfmetrics import stringWidth
from reportlab.lib.utils import instantiated
...
##        return float(stringWidth(word, self.fontName, self.fontSize))
    def fitLine(self, program, totalLength):
        ...
        while not done and cursor<maxcursor:
            ...
            if isinstance(opcode,str) or instantiated(opcode):
...
##                    print "AT END OF PROGRAM"
    ...
    def insertShift(self, line, shift):
        ...
        first = 1
        for e in line:
            if first and (isinstance(e,str) or instantiated(e)):
                result.append(shift)
                first = 0
            result.append(e)
        return result
    def justifyAlign(self, line, lineLength, maxLength):
        ...
        for e in line:
            ...
            elif isinstance(e,str) or instantiated(e):
                visible = 1
        ...
        while cursor<nline:
            ...
            te = type(e)
            result.append(e)
            if isinstance(e,str) or instantiated(e):
                visible = 1
    ...
    def shrinkWrap(self, line):
        ...
        while index<maxindex:
            ...
            if isinstance(e,str) and index<maxindex-1:
                ...
                index = index+1
                nexte = line[index]
                while index<maxindex and isinstance(nexte,(str,float)):
...
##            e = result[count]
##            if isinstance(e,(str,tuple)) or instantiated(e):
##                done = 1
...
##            count = count-1
        ...
        while change:
            ...
            for index in rline:
                ...
                doswap = 0
                # don't swap visibles
                if isinstance(this,str) or isinstance(next,str) or instantiated(this) or instantiated(next):
                    doswap = 0
...
class StyleAttributeConverters:
    fontSize=[rml2pdf.readLength]
    leading=[rml2pdf.readLength]
    leftIndent=[rml2pdf.readLength]
    rightIndent=[rml2pdf.readLength]
    firstLineIndent=[rml2pdf.readLength]
    alignment=[rml2pdf.readAlignment]
    spaceBefore=[rml2pdf.readLength]
    spaceAfter=[rml2pdf.readLength]
    bulletFontSize=[rml2pdf.readLength]
    bulletIndent=[rml2pdf.readLength]
    textColor=[rml2pdf.readColor]
    backColor=[rml2pdf.readColor]
...
class Para(Flowable):
    ...
    def compileProgram(self, parsedText, program=None):
        ...
        if style.firstLineIndent:
            ...
            for x in program:
                count = count+1
                if isinstance(x,str) or instantiated(x):
                    break
...
class paraMapper(MapNode):
    ...
    def translate(self, nodetuple, controller, context, overrides):
        ...
        if not bulletText and len(content)==1:
            text = content[0]
            if type(text) is bytes and "&" not in text:
...
def test2(canv):
    ...
    parsedpara = xml2TT(testparagraph+testparagraph+testparagraph)
    S = ParagraphStyle("Normal", None)
    P = Para(S, parsedpara)
    (w, h) = P.wrap(inch, 3*inch)
    print("wrapped as", (h,w))
    canv.translate(inch, 3*inch)
    canv.rect(0,0,inch,3*inch, fill=0, stroke=1)
    P.canv = canv
    P.draw()
